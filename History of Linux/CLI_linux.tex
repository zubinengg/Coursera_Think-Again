...
\section{Unblock yourself in the command line}
This section explains how to get back to the command prompt, even if you are stuck inside a program.

\subsection{Basic Exit Strategies}
There are several standard ways to exit a program or an input mode:
\begin{itemize}
    \item \textbf{Ctrl+C:} This is the most common way to interrupt and terminate a running program (e.g., the \texttt{rain} program). It works in most, but not all, cases.
    \item \textbf{Program-Specific Commands:} Some programs have their own exit commands. For example, in \texttt{vim}, you must press \textbf{Esc} and then type \texttt{:q!} and press \textbf{Enter} to force quit. \textbf{Ctrl+C} will not work.
    \item \textbf{Closing Input:} If you have an open single quote (\texttt{'}), double quote (\texttt{"}), or bracket, the shell will wait for you to close it. You must type the matching character to finish the input and return to the prompt.
    \item \textbf{Ctrl+D (End of File):} This signal is used to indicate the end of input.
          \begin{itemize}
              \item It is used to exit from commands that read from standard input, such as \texttt{cat > file.txt}.
              \item Sometimes it needs to be pressed on a new line, and occasionally it needs to be pressed twice.
          \end{itemize}
\end{itemize}

\subsection{The Worst Case: Killing a Stuck Process}
...
\begin{enumerate}
    \item \textbf{Open a Second Terminal:} In environments like WebLinux, there is often a button (e.g., an arrow) to open a new terminal session.
    \item \textbf{Identify the Process with \texttt{htop}:} In the second terminal, run \texttt{htop}. This will show a list of all running processes for your user.
    \item \textbf{Find the Process:} Locate the stuck program in the list (e.g., \texttt{rain}). Note its \textbf{PID} (Process Identifier), which is a unique number assigned to it.
    \item \textbf{Send the Kill Signal:}
          \begin{itemize}
              \item Use the arrow keys to select the stuck process within \texttt{htop}.
              \item Press the \textbf{F9} key to bring up the "send signal" menu.
              \item Select signal \textbf{9 (SIGKILL)} from the list on the left. This is a non-ignorable signal that forcefully terminates the process.
              \item Press \textbf{Enter} to send the signal.
          \end{itemize}
    \item \textbf{Verify Termination:} The process should disappear from the \texttt{htop} list. If you switch back to the first terminal, you will see a "Killed" message, and the prompt should return.
\end{enumerate}

\subsection{Recovering a Stuck Terminal (WebLinux)}
Sometimes, even after killing a process, the original terminal can become unresponsive.
\begin{itemize}
    \item \textbf{The Bug:} In WebLinux, after killing a process from a second terminal, the prompt in the first terminal may return but not accept any input.
    \item \textbf{Standard Linux Solution:} On a full Linux system, you would typically open another terminal and kill the stuck shell process itself. Reboots are rarely necessary.
    \item \textbf{WebLinux Solution (The Last Resort):} If the terminal is completely stuck, the simplest way to "reboot" WebLinux is to \textbf{reload the browser page}. This will start a fresh Linux environment, but your files will be preserved.
\end{itemize}

\separator

\section{mv: rename and move}
This section explains how to move and rename files and directories using the \texttt{mv} command.

\subsection{Moving Files and Directories}
The \texttt{mv} command is used to move a file or directory from one location to another.
\begin{itemize}
    \item \textbf{Move a file into a directory:}
          \begin{tcolorbox}
              \texttt{\$ touch f1} \\
              \texttt{\$ mkdir d1} \\
              \texttt{\$ mv f1 d1}
          \end{tcolorbox}
          This moves the file \texttt{f1} into the directory \texttt{d1}.
    \item \textbf{Move a directory into another directory:}
          \begin{tcolorbox}
              \texttt{\$ mkdir bigdir} \\
              \texttt{\$ mv d1 bigdir}
          \end{tcolorbox}
          This moves the directory \texttt{d1} (and its contents) into \texttt{bigdir}.
\end{itemize}

\subsection{Using Relative Paths}
You can use relative paths to move items around the filesystem.
\begin{itemize}
    \item \textbf{Moving a file up the directory tree:} If you are inside \texttt{/home/user/bigdir/d1}, you can move \texttt{f1} back to the home directory (\texttt{\textasciitilde}).
          \begin{tcolorbox}
              \texttt{\$ pwd} \\
              \texttt{/home/user/bigdir/d1} \\
              \texttt{\$ mv f1 ../..}
          \end{tcolorbox}
          The path \texttt{../..} refers to the parent of the parent directory, which is \texttt{/home/user}.
    \item \textbf{Error case:} You cannot move the directory you are currently in.
          \begin{tcolorbox}
              \texttt{\$ pwd} \\
              \texttt{/home/user/bigdir/d1} \\
              \texttt{\$ mv . ../..} \\
              \texttt{mv: cannot move '.' to '../../d1': Device or resource busy}
          \end{tcolorbox}
\end{itemize}

\subsection{Renaming with mv}
There is no dedicated "rename" command in Linux. The \texttt{mv} command is used for renaming. If the destination is a new name in the same directory, \texttt{mv} performs a rename.
\begin{tcolorbox}
    \texttt{\$ mv bigdir smalldir}
\end{tcolorbox}
This command renames the directory \texttt{bigdir} to \texttt{smalldir}.

\subsection{Moving and Renaming Simultaneously}
You can combine moving and renaming into a single command.
\begin{itemize}
    \item \textbf{Move and rename a file:} Move \texttt{f1} from \texttt{otherdir} to the home directory and rename it to \texttt{f2}.
          \begin{tcolorbox}
              \texttt{\$ mv otherdir/f1 \textasciitilde/f2}
          \end{tcolorbox}
    \item \textbf{Move and rename a directory:} Move \texttt{d1} from \texttt{smalldir} into \texttt{otherdir} and rename it to \texttt{d2}.
          \begin{tcolorbox}
              \texttt{\$ mv smalldir/d1 otherdir/d2}
          \end{tcolorbox}
\end{itemize}

\subsection{The Dangers of Overwriting}
The \texttt{mv} command can be dangerous because it will overwrite existing files without warning.
\begin{itemize}
    \item Consider two files, \texttt{f5} and \texttt{f6}, with different content.
          \begin{tcolorbox}
              \texttt{\$ echo "this is f5" > f5} \\
              \texttt{\$ echo "this is f6" > f6}
          \end{tcolorbox}
    \item If you "rename" \texttt{f6} to \texttt{f5}, the original \texttt{f5} is permanently lost.
          \begin{tcolorbox}
              \texttt{\$ mv f6 f5} \\
              \texttt{\$ cat f5} \\
              \texttt{this is f6}
          \end{tcolorbox}
          \textbf{Warning:} The original content of \texttt{f5} has been overwritten and cannot be recovered.
\end{itemize}

\subsection{Safe Moving and Renaming}
To prevent accidental overwriting, you can use options with the \texttt{mv} command. You can see these options with \texttt{mv --help}.
\begin{itemize}
    \item \textbf{Interactive Mode (\texttt{-i}):} This option prompts you for confirmation before overwriting a file.
          \begin{tcolorbox}
              \texttt{\$ mv -i f6 f5} \\
              \texttt{mv: overwrite 'f5'? n}
          \end{tcolorbox}
          You can type \texttt{n} (for no) to cancel the operation and prevent data loss.
    \item \textbf{No-Clobber Mode (\texttt{-n}):} This option prevents \texttt{mv} from overwriting any existing file. The move will simply not happen if the destination file exists.
\end{itemize}
\textbf{Best Practice:} Be very careful with \texttt{mv}. Using the \texttt{-i} option is a good habit to avoid losing data.

\separator

\section{cp, cp -r}

\subsection{Creating Files with cat (Recap)}
\begin{itemize}
    \item We can create a file using \texttt{cat} and redirection.
    \item \texttt{cat > f1}: Enters input mode. Type content (e.g., "hello"), then press \textbf{Ctrl+D} to save and exit.
    \item \texttt{cat f1}: Displays the content ("hello").
\end{itemize}

\subsection{Copying Files (cp)}
\begin{itemize}
    \item \texttt{cp f1 f2}: Copies content of \texttt{f1} to a new file \texttt{f2}.
    \item \texttt{cat f2}: Confirms \texttt{f2} contains "hello".
\end{itemize}

\subsection{Copying via Redirection}
\begin{itemize}
    \item You can also copy by redirecting \texttt{cat} output.
    \item \texttt{cat f2 > f3}: Reads \texttt{f2} and redirects output to \texttt{f3}.
    \item This effectively copies \texttt{f2} to \texttt{f3}.
\end{itemize}

\subsection{Copying Directories (Recursive)}
\begin{itemize}
    \item \texttt{cp d1 d2}: Tries to copy directory \texttt{d1} to \texttt{d2}.
    \item \textbf{Error:} "omitting directory d1".
    \item \textbf{Solution:} Use the recursive option \texttt{-r}.
    \item \texttt{cp -r d1 d2}: Successfully copies \texttt{d1} into \texttt{d2}.
\end{itemize}

\subsection{Recursive Copy Behavior}
\begin{itemize}
    \item If \texttt{d2} does not exist: \texttt{cp -r d1 d2} creates \texttt{d2} and copies contents of \texttt{d1} into it.
    \item If \texttt{d2} exists: \texttt{cp -r d1 d2} copies \texttt{d1} \textit{inside} \texttt{d2}.
    \item Doing it again will stick another copy or overwrite existing files inside the destination.
\end{itemize}

\subsection{Safety and Overwriting}
\begin{itemize}
    \item \texttt{cp} can overwrite text indiscriminately.
    \item \texttt{cp --help}: Shows options.
    \item \textbf{-i (Interactive)}: Prompts before overwriting.
    \item \textbf{-f (Force)}: Forces overwrite (default behavior usually).
    \item Example: \texttt{cp -ri d1 d2} -> Prompts "overwrite d2/d1/f1?". Answering "no" prevents accidental data loss.
\end{itemize}
...
\subsection{The locate Command}
In a real Linux environment, you would typically use the \texttt{locate} command to search for files.
\begin{itemize}
    \item It relies on a database of all files in the system.
    \item You update this database with the \texttt{updatedb} command.
    \item \textbf{Note:} On WebLinux, \texttt{locate} is not installed to keep the system light and fast.
\end{itemize}

\subsection{Simple Search with Wildcards}
A simple way to find files in a specific directory is using the \texttt{echo} command with shell wildcards.
\begin{itemize}
    \item \textbf{Star (*):} Expands to any number of characters.
          \begin{tcolorbox}
              \texttt{\$ echo p*}
          \end{tcolorbox}
          This prints all files starting with 'p' (e.g., \texttt{program.c}).
    \item \textbf{Question Mark (?):} Expands to exactly one character.
          \begin{tcolorbox}
              \texttt{\$ echo /usr/bin/a??}\\
              \texttt{\$ echo /usr/bin/???}
          \end{tcolorbox}
          This lists files in \texttt{/usr/bin} with starting withg letter 'a' and two letters (e.g., \texttt{abc}, \texttt{add}, \texttt{adb}).\\
          This lists files in \texttt{/usr/bin} with exactly three letters (e.g., \texttt{cal}, \texttt{man}, \texttt{vim}).
\end{itemize}

\subsection{The find Command}
The \texttt{find} command searches for files within the directory hierarchy.

\subsubsection{Basic Usage}
\begin{itemize}
    \item \texttt{find} or \texttt{find .}: Lists all files in the current directory recursively.
    \item \texttt{find /}: Lists all files in the entire system (root).
\end{itemize}

\subsubsection{Searching by Name}
Use \texttt{-name} to search for a specific filename.
\begin{tcolorbox}
    \texttt{\$ find . -name "program.c"}
\end{tcolorbox}

\subsubsection{Using Patterns}
You can use patterns with wildcards. \textbf{Always use double quotes} around the pattern to prevent shell expansion.
\begin{tcolorbox}
    \texttt{\$ find . -name "prog*"}
\end{tcolorbox}

\subsubsection{Handling Errors (Redirecting to /dev/null)}
Searching the whole system often results in "Permission denied" errors. You can redirect these error messages (stream 2) to \texttt{/dev/null} (the system's "black hole").
\begin{tcolorbox}
    \texttt{\$ find / -name "Hello" 2> /dev/null}
\end{tcolorbox}

\subsubsection{Advanced Search Options}
\begin{itemize}
    \item \textbf{Case Insensitive (-iname):} Ignores case distinctions.
          \begin{tcolorbox}
              \texttt{\$ find / -iname "*joe*" 2> /dev/null}
          \end{tcolorbox}
    \item \textbf{Searching Paths (-ipath):} Searches within the full path, not just the filename.
          \begin{tcolorbox}
              \texttt{\$ find / -ipath "*joe*" 2> /dev/null}
          \end{tcolorbox}
\end{itemize}

\subsubsection{Piping to less}
If the result list is long, pipe it to \texttt{less} to scroll through it.
\begin{tcolorbox}
    \texttt{\$ find / -ipath "*joe*" 2> /dev/null | less}
\end{tcolorbox}
