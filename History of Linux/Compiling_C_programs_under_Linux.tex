...
\begin{tcolorbox}
    ...
    \texttt{\hspace*{0.5cm} double max = temperatures[0];} \\
    \texttt{\hspace*{0.5cm} int i;} \\
    \texttt{\hspace*{0.5cm} for (i = 1; i < size; i++) \{} \\
    \texttt{\hspace*{1cm} if (max < temperatures[i]) \{} \\
    \texttt{\hspace*{1.5cm} max = temperatures[i];} \\
    \texttt{\hspace*{1cm} \}} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{\hspace*{0.5cm} return max;} \\
    \texttt{\}}
\end{tcolorbox}
...
\section{Modify your library}

In this section, we will look at how to make modifications to a static library and to a program that uses it. We assume we have \texttt{program.c} and \texttt{weatherstats.c}, but we haven't created the library \texttt{libweather.a} yet.

\subsection{Creating the Initial Library}
First, we compile the existing module \texttt{weatherstats.c} into an object file.
\begin{tcolorbox}
    \texttt{gcc -c weatherstats.c -o weatherstats.o}
\end{tcolorbox}

Then, we create the static library archive \texttt{libweather.a}.
\begin{tcolorbox}
    \texttt{ar rcs libweather.a weatherstats.o}
\end{tcolorbox}

\subsection{Adding a New Module}
We want to add a function that prints out temperatures. We create a new source file named \texttt{weatherio.c}.

\subsubsection{The Source Code (weatherio.c)}
\begin{tcolorbox}
    \texttt{\#include <stdio.h>} \\
    \texttt{} \\
    \texttt{void printTemps(double *temperatures, int size) \{} \\
    \texttt{\hspace*{0.5cm} printf("Over the past \%d days the temperatures were:\textbackslash n", size);} \\
    \texttt{\hspace*{0.5cm} int i;} \\
    \texttt{\hspace*{0.5cm} for (i = 0; i < size; i++) \{} \\
    \texttt{\hspace*{1cm} printf("On day \%d the temperature was \%.2lf\textbackslash n", i + 1, temperatures[i]);} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{\}}
\end{tcolorbox}

\subsubsection{The Header File (weatherio.h)}
We also create the header file with the function prototype.
\begin{tcolorbox}
    \texttt{void printTemps(double *temperatures, int size);}
\end{tcolorbox}

\subsubsection{Compiling the New Module}
We compile this new file into an object file.
\begin{tcolorbox}
    \texttt{gcc -c weatherio.c -o weatherio.o}
\end{tcolorbox}

\subsection{Updating the Library}
Now we add the new object file to our existing library. We can simply run the \texttt{ar} command again with all the object files we want in the library.
\begin{tcolorbox}
    \texttt{ar rcs libweather.a weatherstats.o weatherio.o}
\end{tcolorbox}
The library \texttt{libweather.a} now contains both \texttt{weatherstats.o} and \texttt{weatherio.o}.

\subsection{Using the Updated Library}
To use the new function in our main program, we need to update \texttt{program.c} and manage our headers.

\subsubsection{Creating a Master Header (weather.h)}
Instead of including multiple header files in \texttt{program.c}, it is cleaner to create a single header file for the library that includes the others.
\begin{tcolorbox}
    \texttt{\#include "weatherstats.h"} \\
    \texttt{\#include "weatherio.h"}
\end{tcolorbox}

\subsubsection{Updating the Main Program}
In \texttt{program.c}, we include the new master header and call the \texttt{printTemps} function.
\begin{tcolorbox}
    \texttt{\#include "weather.h"} \\
    \texttt{...} \\
    \texttt{printTemps(temperatures, 7);}
\end{tcolorbox}

\subsubsection{Compiling and Linking}
We can compile the program and link it with the library in a single command. We use \texttt{-L.} to tell GCC to look in the current directory for libraries, and \texttt{-lweather} to link against \texttt{libweather.a}.
\begin{tcolorbox}
    \texttt{gcc program.c -L. -lweather -o program}
\end{tcolorbox}
Running \texttt{./program} will now display the temperatures using the new function from the updated library.

\separator

\section{Ultimate makefile}

Let's finish this section on libraries by updating our previous Makefiles. We assume that our library \texttt{libweather.a} exists and is unchanged. Therefore, our Makefile will only be concerned with creating the executable program.

\subsection{Updating the Simple Makefile}
The program no longer depends on \texttt{weatherstats.o} because we are using the library. We update the build command to include the library path (\texttt{-L.}) and the library itself (\texttt{-lweather}).

\begin{tcolorbox}
    \texttt{program: program.o} \\
    \texttt{\hspace*{1cm} gcc -std=C11 -Wall -fmax-errors=10 -Wextra -o program program.o -L. -lweather} \\
    \texttt{} \\
    \texttt{program.o: program.c} \\
    \texttt{\hspace*{1cm} gcc -std=C11 -Wall -fmax-errors=10 -Wextra -c program.c -o program.o} \\
    \texttt{} \\
    \texttt{launch: program} \\
    \texttt{\hspace*{1cm} ./program}
\end{tcolorbox}

When we run \texttt{make launch}, \texttt{make} will check dependencies: it builds \texttt{program.o} from \texttt{program.c}, then builds \texttt{program} by linking \texttt{program.o} with the library, and finally executes it.

\subsection{Updating the Elaborate Makefile}
...
\begin{tcolorbox}
    ...
    \texttt{} \\
    \texttt{\$(MAIN): \$(OBJ)} \\
    \texttt{\hspace*{1cm} \$(CC) \$(CFLAGS) -o \$(MAIN) \$(OBJ) \$(LFLAGS) \$(LIBS)} \\
    \texttt{} \\
    \texttt{\%.o: \%.c} \\
    \texttt{\hspace*{1cm} \$(CC) \$(CFLAGS) -c \$< -o \$@} \\
    \texttt{} \\
    \texttt{launch: \$(MAIN)} \\
    \texttt{\hspace*{1cm} ./\$(MAIN)} \\
    \texttt{} \\
    \texttt{clean:} \\
    \texttt{\hspace*{1cm} rm *.o \$(MAIN)}
\end{tcolorbox}

This Makefile is versatile. You can use it for other projects by simply updating the variables at the top. The \texttt{clean} target allows you to start from a clean slate by removing object files and the executable.

\separator

\section{Pass arguments to a program from the command line}

In this section, we will learn how to pass arguments to a C program directly from the command line, similar to how we pass options to standard commands like \texttt{ls -l}.

\subsection{Modifying the main function}
To accept command-line arguments, we need to modify the signature of the \texttt{main} function. Instead of \texttt{int main(void)} or empty parentheses, we use:

\begin{tcolorbox}
    \texttt{int main(int argc, char *argv[])}
\end{tcolorbox}

Alternatively, you can write \texttt{char **argv}. This emphasizes that \texttt{argv} is a pointer to a pointer (a matrix or array of strings).

\subsection{Understanding argc and argv}
\begin{itemize}
    \item \textbf{argc (Argument Count):} An integer representing the number of arguments passed.
    \item \textbf{argv (Argument Vector):} An array of strings (pointers to characters) containing the actual arguments.
\end{itemize}

\textbf{Important Note:} \texttt{argc} is always at least 1 because the name of the program itself is counted as the first argument (at index 0).

\subsection{Example Program}
Here is a program that prints all the arguments passed to it:

\begin{tcolorbox}
    \texttt{\#include <stdio.h>} \\
    \texttt{} \\
    \texttt{int main(int argc, char **argv) \{} \\
    \texttt{\hspace*{0.5cm} printf("I have \%d arguments from the command line\textbackslash n", argc);} \\
    \texttt{\hspace*{0.5cm} int i;} \\
    \texttt{\hspace*{0.5cm} for (i = 0; i < argc; i++) \{} \\
    \texttt{\hspace*{1cm} printf("Argument \%d: \%s\textbackslash n", i, argv[i]);} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{    return 0;} \\
    \texttt{\}}
\end{tcolorbox}

If we run \texttt{./program -e -t}, \texttt{argc} will be 3. Argument 0 is \texttt{./program}, argument 1 is \texttt{-e}, and argument 2 is \texttt{-t}.

\separator

\section{Use arguments passed to a program from the command line}

In this section, we will look at a program that allows you to recover arguments of different types passed to your main function from the command line.

\subsection{The Problem: All Arguments are Strings}
When you pass arguments like \texttt{-e}, \texttt{43}, and \texttt{27.88} to your program, they are all received as strings (arrays of characters) in the \texttt{argv} array.
\begin{itemize}
    \item \texttt{-e} is a string.
    \item \texttt{43} is received as the string "43", not the number 43.
    \item \texttt{27.88} is received as the string "27.88".
\end{itemize}
To use these as numbers (e.g., to do math), we must convert them.

\subsection{Tools for Conversion and Comparison}
We use specific functions from the standard libraries to handle these conversions.
\begin{itemize}
    \item \textbf{strcmp:} Defined in \texttt{<string.h>}. It compares two strings. It returns \texttt{0} if the strings are identical.
    \item \textbf{atoi:} Defined in \texttt{<stdlib.h>}. Stands for "ASCII to Integer". It converts a string to an \texttt{int}.
    \item \textbf{atof:} Defined in \texttt{<stdlib.h>}. Stands for "ASCII to Float". It converts a string to a \texttt{double}.
\end{itemize}

\subsection{Example Program}
Here is a program that checks for a specific flag and converts numerical arguments.

\begin{tcolorbox}
    \texttt{\#include <stdio.h>} \\
    \texttt{\#include <string.h>} \\
    \texttt{\#include <stdlib.h>} \\
    \texttt{} \\
    \texttt{int main(int argc, char **argv) \{} \\
    \texttt{\hspace*{0.5cm} printf("I have \%d arguments from the command line\textbackslash n", argc);} \\
    ...
    \texttt{\hspace*{1cm} printf("Argument \%d: \%s\textbackslash n", i, argv[i]);} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{\hspace*{0.5cm} if (argc == 4) \{} \\
    \texttt{\hspace*{1cm} int compare = strcmp(argv[1], "-e");} \\
    \texttt{\hspace*{1cm} if (compare == 0) printf("argv[1] equals -e\textbackslash n");} \\
    \texttt{\hspace*{1cm} else printf("argv[1] does not equal -e\textbackslash n");} \\
    \texttt{\hspace*{1cm} int wholeNumber = atoi(argv[2]);} \\
    \texttt{\hspace*{1cm} printf("Argument 2 as integer: \%d\textbackslash n", wholeNumber);} \\
    \texttt{\hspace*{1cm} double decimalNumber = atof(argv[3]);} \\
    \texttt{\hspace*{1cm} printf("Argument 3 as double: \%lf\textbackslash n", decimalNumber);} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{    return 0;} \\
    \texttt{\}}
\end{tcolorbox}
...
\section{Read numbers from a file}

So far, whenever one of our programs needed a user input, such as a temperature to be converted from Celsius to Fahrenheit or a student's grade, we have the user enter this input at program execution time in the terminal window. Now imagine you have a lot of data to be entered, be it to compute the average grade in a class for lots of students or something else. In this case, it may be more efficient and easier to read this data in automatically from a file you created previously.

\subsection{The Three Steps}
There are three steps when reading from a file:
\begin{enumerate}
    \item \textbf{Open:} You need to open the file.
    \item \textbf{Read:} You read from the file.
    \item \textbf{Close:} When you are done, you need to close the file.
\end{enumerate}

\subsection{File Structure}
For this example, we will use a file named \texttt{my\_first\_file.txt}. It is important that there are no spaces in your file names (use underscores instead).
To simplify reading until the end of the file, we will use a structure where the first entry is the number of entries that follow.
\begin{itemize}
    \item \textbf{Example Content:}
          \begin{tcolorbox}
              9 \\
              1 2 3 4 5 6 7 8 9
          \end{tcolorbox}
    \item The \texttt{9} indicates that there are nine integers on the next line following.
\end{itemize}

\subsection{Opening a File}
To read from a file in C, we use a file pointer.
\begin{itemize}
    \item \textbf{File Pointer:} A pointer of type \texttt{FILE} (defined in \texttt{stdio.h}) used to handle and keep track of the file being accessed.
    \item \textbf{fopen:} The function used to open a file. It returns a file pointer.
    \item \textbf{Syntax:} \texttt{FILE *ifile = fopen("my\_first\_file.txt", "r");}
          \begin{itemize}
              \item The first parameter is the filename in double quotes.
              \item The second parameter is \texttt{"r"} for \textbf{reading}. (We use \texttt{"w"} for writing and \texttt{"a"} for appending).
          \end{itemize}
\end{itemize}

\subsection{Reading from a File}
We use the \texttt{fscanf} command to read from the file.
\begin{itemize}
    \item \textbf{Comparison:} It is just like \texttt{scanf}, but has an extra \texttt{f} in front (file scanf).
    \item \textbf{Arguments:} It takes an extra first argument: the file pointer.
    \item \textbf{Example:} \texttt{fscanf(ifile, "\%d", \&N);} reads an integer from the file into variable \texttt{N}.
\end{itemize}

\subsection{Closing a File}
We must always close a file when done to prevent data loss.
\begin{itemize}
    \item \textbf{Command:} \texttt{fclose(ifile);}
\end{itemize}

\subsection{Example Program: Reading Integers}
Here is a program that reads the count of numbers and then loops to read the rest.
\begin{tcolorbox}
    \texttt{\#include <stdio.h>} \\
    \texttt{} \\
    \texttt{int main(void) \{} \\
    \texttt{\hspace*{0.5cm} FILE *ifile;} \\
    \texttt{\hspace*{0.5cm} int N, i, number;} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} ifile = fopen("my\_first\_file.txt", "r");} \\
    \texttt{\hspace*{0.5cm} fscanf(ifile, "\%d", \&N);} \\
    \texttt{\hspace*{0.5cm} printf("There are \%d numbers in the file.\textbackslash n", N);} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} for (i = 0; i < N; i++) \{} \\
    \texttt{\hspace*{1cm} fscanf(ifile, "\%d", \&number);} \\
    \texttt{\hspace*{1cm} printf("I read \%d from the file.\textbackslash n", number);} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} fclose(ifile);} \\
    \texttt{    return 0;} \\
    \texttt{\}}
\end{tcolorbox}

\subsection{Example Program: Reading Doubles}
You can also read doubles. Create a file \texttt{my\_second\_file.txt} with doubles (e.g., \texttt{4.5 6.2 ...}).
\begin{itemize}
    \item Change the variable type to \texttt{double}.
    \item Change the format specifier in \texttt{fscanf} and \texttt{printf} to \texttt{\%lf}.
\end{itemize}

\separator

\section{Read until the end of the file}

In this section, we will learn how to read from a file when we do not know beforehand how many numbers are stored in it. In the previous example, the first number in the file indicated the count, but often we want to simply read until the end of the file.

\subsection{Creating the Data File}
We want to create a new file named \texttt{my\_third\_file.txt} that contains numbers without an initial count.
\begin{tcolorbox}
    \texttt{touch my\_third\_file.txt}
\end{tcolorbox}
Edit this file and paste in some numbers (e.g., 56, 3, ...) without the leading count.

\subsection{Using a while Loop}
Since we do not know the number of iterations required, a \texttt{for} loop is not suitable. A \texttt{while} loop is better suited for this task.

\subsubsection{Checking for EOF}
The \texttt{fscanf} function returns a value indicating the number of items successfully read. If it reaches the end of the file, it returns the constant \texttt{EOF} (End Of File), which is defined in \texttt{stdio.h}.

We can use a variable \texttt{message} to store this return value and a flag \texttt{done} to control the loop.
\begin{tcolorbox}
    \texttt{int done = 0;} \\
    \texttt{int message;} \\
    \texttt{while (!done) \{} \\
    \texttt{\hspace*{0.5cm} message = fscanf(ifile, "\%d", \&num);} \\
    \texttt{\hspace*{0.5cm} if (message == EOF) \{} \\
    \texttt{\hspace*{1cm} done = 1;} \\
    \texttt{\hspace*{0.5cm} \} else \{} \\
    \texttt{\hspace*{1cm} printf("I read \%d from file\textbackslash n", num);} \\
    \texttt{\hspace*{1cm} N++;} \\
    \texttt{\hspace*{1cm} sum += num;} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{\}}
\end{tcolorbox}

\subsection{Calculating the Average}
To calculate the average, we sum the numbers and divide by the count \texttt{N}.
\begin{itemize}
    \item \textbf{Integer Division:} If you divide two integers (e.g., \texttt{sum / N}), C performs integer division, resulting in an integer.
    \item \textbf{Casting:} To get a precise floating-point result, you must cast one of the operands to \texttt{double}.
          \begin{tcolorbox}
              \texttt{printf("The average is \%.2lf\textbackslash n", (double)sum / N);}
          \end{tcolorbox}
\end{itemize}

\subsection{Optimized Loop Syntax}
We can combine the reading and the EOF check into the \texttt{while} condition. This is a common and concise idiom in C.
\begin{tcolorbox}
    \texttt{while (fscanf(ifile, "\%d", \&num) != EOF) \{} \\
    \texttt{\hspace*{0.5cm} printf("I read \%d from the file\textbackslash n", num);} \\
    \texttt{\hspace*{0.5cm} N++;} \\
    \texttt{\hspace*{0.5cm} sum += num;} \\
    \texttt{\}}
\end{tcolorbox}
\textbf{Note:} When using this method, do not place a second \texttt{fscanf} inside the loop body, as the reading happens in the condition.

\separator

\section{Write numbers to a file using fprintf()}

Now that you know how to read input from files, it is easy to learn how to write output to files. Suppose we wanted the user to enter student grades from the keyboard and then store these grades in a file.

\subsection{Opening a File for Writing}
To write to a file, we declare a file pointer (e.g., \texttt{ofile} for output file) and use \texttt{fopen} with the \texttt{"w"} mode.
\begin{itemize}
    \item \textbf{Mode "w":} Tells the computer we want to write to the file.
    \item \textbf{Overwriting:} If the file already exists, opening it with \texttt{"w"} will overwrite it, and you will lose any previous content. If it does not exist, it will be created.
\end{itemize}

\subsection{Writing with fprintf}
The \texttt{fprintf} command works just like \texttt{printf}, except we also pass in the file pointer.
\begin{itemize}
    \item \textbf{Syntax:} \texttt{fprintf(file\_pointer, "format", variables);}
    \item \textbf{Example:} \texttt{fprintf(ofile, "\%d ", number);}
\end{itemize}

\subsection{Formatting Output}
It is important to include separators (like spaces or newlines) in the format string.
\begin{itemize}
    \item \textbf{The Problem:} If you write numbers without spaces (e.g., 100, 99), they will be "smushed" together in the file (e.g., "10099"), making them impossible to distinguish when reading back.
    \item \textbf{The Solution:} Add a space in the format string: \texttt{"\%d "}.
\end{itemize}

\subsection{Example Program}
This program asks the user to enter grades and writes them to a file. It uses a \texttt{do-while} loop and stops when the user enters -1. It also demonstrates using a variable for the filename.

\begin{tcolorbox}
    \texttt{\#include <stdio.h>} \\
    \texttt{} \\
    \texttt{int main(void) \{} \\
    \texttt{\hspace*{0.5cm} FILE *ofile;} \\
    \texttt{\hspace*{0.5cm} char filename[] = "my\_first\_output\_file.txt";} \\
    \texttt{\hspace*{0.5cm} int num;} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} ofile = fopen(filename, "w");} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} do \{} \\
    \texttt{\hspace*{1cm} printf("Please enter a grade. Enter -1 to quit: \textbackslash n");} \\
    \texttt{\hspace*{1cm} scanf("\%d", \&num);} \\
    \texttt{\hspace*{1cm} if (num != -1) \{} \\
    \texttt{\hspace*{1.5cm} fprintf(ofile, "\%d ", num);} \\
    \texttt{\hspace*{1cm} \}} \\
    \texttt{\hspace*{0.5cm} \} while (num != -1);} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} fclose(ofile);} \\
...
\section{Append numbers to a file at the end}

In the previous example, we opened the file using the \texttt{"w"} mode. This mode creates a new file if it does not exist, but if the file already exists, it overwrites it, destroying previous data.

\subsection{The Append Mode ("a")}
To add data to the end of an existing file without losing the current content, we use the \texttt{"a"} (append) mode.
\begin{itemize}
    \item \textbf{Behavior:} If the file exists, new data is added to the end. If the file does not exist, a new one is created (just like \texttt{"w"}).
    \item \textbf{Syntax:} \texttt{ofile = fopen(filename, "a");}
\end{itemize}

\subsection{Checking if a File Exists}
Sometimes you want to give the user a choice: append to the existing file or overwrite it. To do this, you first need to check if the file exists.
\begin{itemize}
    \item \textbf{The Trick:} Try to open the file for reading (\texttt{"r"}).
    \item \textbf{Result:}
          \begin{itemize}
              \item If \texttt{fopen} returns \texttt{NULL}, the file does not exist.
              \item If it returns a valid pointer, the file exists.
          \end{itemize}
    \item \textbf{Important:} If the file exists, close it immediately after the check before reopening it in the desired mode.
\end{itemize}

\subsection{Example Program: Smart File Handling}
This program checks if the file exists. If it does, it asks the user whether to append or overwrite.
...
\begin{tcolorbox}
    ...
    \texttt{\hspace*{0.5cm} char filename[] = "my\_first\_output\_file.txt";} \\
    \texttt{\hspace*{0.5cm} int num;} \\
    \texttt{\hspace*{0.5cm} int selection = 1;} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} // Check if file exists} \\
    \texttt{\hspace*{0.5cm} ofile = fopen(filename, "r");} \\
    \texttt{\hspace*{0.5cm} if (ofile != NULL) \{} \\
    \texttt{\hspace*{1cm} printf("You already have a file named \%s\textbackslash n", filename);} \\
    \texttt{\hspace*{1cm} fclose(ofile);} \\
    \texttt{\hspace*{1cm} printf("Do you wish to (1) append or (2) overwrite? Enter 1 or 2: ");} \\
    \texttt{\hspace*{1cm} scanf("\%d", \&selection);} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} if (selection == 1) \{} \\
    \texttt{\hspace*{1cm} ofile = fopen(filename, "a");} \\
    \texttt{\hspace*{0.5cm} \} else \{} \\
    \texttt{\hspace*{1cm} ofile = fopen(filename, "w");} \\
    \texttt{\hspace*{0.5cm} \}} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} do \{} \\
    \texttt{\hspace*{1cm} printf("Enter a grade (-1 to quit): ");} \\
    \texttt{\hspace*{1cm} scanf("\%d", \&num);} \\
    \texttt{\hspace*{1cm} if (num != -1) fprintf(ofile, "\%d ", num);} \\
    \texttt{\hspace*{0.5cm} \} while (num != -1);} \\
    \texttt{} \\
    \texttt{\hspace*{0.5cm} fclose(ofile);} \\
    \texttt{\hspace*{0.5cm} return 0;} \\
