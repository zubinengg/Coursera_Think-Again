...
\begin{tcolorbox}
    ...
    \texttt{    printf("Over the past \%d days the temperatures were:\textbackslash n", size);} \\
    \texttt{    int i;} \\
    \texttt{    for (i = 0; i < size; i++) \{} \\
    \texttt{        printf("On day \%d the temperature was \%.2lf\textbackslash n", i + 1, temperatures[i]);} \\
    \texttt{    \}} \\
    \texttt{\}}
\end{tcolorbox}
...
\begin{tcolorbox}
    ...
    \texttt{} \\
    \texttt{int main(int argc, char **argv) \{} \\
    \texttt{    printf("I have \%d arguments from the command line\textbackslash n", argc);} \\
    \texttt{    int i;} \\
    \texttt{    for (i = 0; i < argc; i++) \{} \\
    \texttt{        printf("Argument \%d: \%s\textbackslash n", i, argv[i]);} \\
    \texttt{    \}} \\
    \texttt{    return 0;} \\
    \texttt{\}}
\end{tcolorbox}
...
\begin{tcolorbox}
    ...
    \texttt{    \}} \\
    \texttt{    if (argc == 4) \{} \\
    \texttt{        int compare = strcmp(argv[1], "-e");} \\
    \texttt{        if (compare == 0) printf("argv[1] equals -e\textbackslash n");} \\
    \texttt{        else printf("argv[1] does not equal -e\textbackslash n");} \\
    \texttt{        int wholeNumber = atoi(argv[2]);} \\
    \texttt{        printf("Argument 2 as integer: \%d\textbackslash n", wholeNumber);} \\
    \texttt{        double decimalNumber = atof(argv[3]);} \\
    \texttt{        printf("Argument 3 as double: \%lf\textbackslash n", decimalNumber);} \\
    \texttt{    \}} \\
    \texttt{    return 0;} \\
    \texttt{\}}
\end{tcolorbox}
